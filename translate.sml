...
struct
  ...
  datatype frag = PROC of { body : Tree.stm, frame : F.frame}
                | STRING of Temp.label * string
  ...
  fun xlString s = 
    let
      val isFragPresent = ref false
      fun checkStringFrag frag =
        case frag 
          of PROC(_) => false
           | STRING(lbl, str) => String.compare(s, str) = EQUAL
      val label = 
        case (List.find checkStringFrag (!frags))
          of SOME(STRING(lbl', str')) => lbl'
          |  _ => 
              let 
                val lab = M.newlabel ()
              in
                addFrag (STRING (lab, s)); lab
              end
    in
      Ex (R.NAME label)
    end
  ...
  fun xlArith (A.PlusOp, Ex (R.CONST 0), r) = r
    | xlArith (A.PlusOp, l, Ex (R.CONST 0)) = l
    | xlArith (A.PlusOp, Ex (R.CONST l), Ex (R.CONST r)) = xlInt(l + r)
    | xlArith (A.MinusOp, l, Ex (R.CONST 0)) = l
    | xlArith (A.MinusOp, Ex (R.CONST 0), Ex (R.CONST r)) = xlInt(0 - r)
    | xlArith (A.MinusOp, Ex (R.CONST l), Ex (R.CONST r)) = xlInt(l - r)
    | xlArith (A.TimesOp, Ex (R.CONST 0), r) = Ex(ZERO)
    | xlArith (A.TimesOp, l, Ex (R.CONST 0)) = Ex(ZERO)
    | xlArith (A.TimesOp, Ex (R.CONST l), Ex (R.CONST r)) = xlInt(l * r)
    | xlArith (A.DivideOp, Ex (R.CONST 0), r) = Ex(ZERO)
    | xlArith (A.DivideOp, Ex (R.CONST l), Ex (R.CONST r)) = xlInt(l div r)
    | xlArith (ao, l, r) = Ex (R.BINOP (aop2binop ao,  unEx l, unEx r))
