...
struct
    ...
    val base_tenv =
        let fun enter ((name, ty), tenv) = S.enter ( tenv, S.symbol name, ty)
        in foldl enter S.empty [("bool", T.BOOL), ("int", T.INT), ("string", T.STRING), ("unit", T.UNIT)]
        end

    val base_venv =
        let 
          fun enter ((name, formals, result), venv) = 
            S.enter ( venv, S.symbol name, FunEntry { level = X.outermost, label = M.namedlabel name, formals = formals, result = result})
